<script>
import axios from 'axios';
import Global from './Global.js';
export default {
  data() {
    return {
      el_s2: null,
      el_s3: null,
      isShow: true,
      isShow_s2: false,
      isShow_s3: false,
      roomID: null,
      roomID2: null,
      allrooms: [
        {
          "name": "test",
          "code": "MC4yNj",
          "admins": [
            "63babf42102799dada53bc11"
          ],
          "createdBy": "63babf42102799dada53bc11",
          "createdAt": "2023-01-08T15:16:01.346Z",
          "updatedAt": "2023-01-08T15:16:01.346Z",
          "id": "63bade318b1be06bd2358e0b"
        }
      ]
    };
  },
  methods: {
    isScrolledIntoView(el) {
      var rect = el.getBoundingClientRect();
      var elemTop = rect.top;
      var elemBottom = rect.bottom;
      var isVisible = elemTop + 500 < window.innerHeight && elemBottom >= 0;
      return isVisible;
    },
    handleScroll() {
      /* if scrolled into view */
      if (this.isScrolledIntoView(this.el_s2)) {
        this.isShow_s2 = true;
      }
      if (this.isScrolledIntoView(this.el_s3)) {
        this.isShow_s3 = true;
      }
    },
    find_room_and_redirect(code) {
      let flag = false;
      for (let i = 0; i < this.allrooms.length; i++) {
        if (this.allrooms[i].code === code) {
          window.location.href = `/join/${this.allrooms[i].id}`;
          flag = true;
        }
      }
      if (flag === false)
        alert('房號輸入錯誤或未存在');
    }
  },
  created() {
    window.addEventListener("scroll", this.handleScroll);
  },
  mounted() {
    this.el_s2 = document.getElementById("s2");
    this.el_s3 = document.getElementById("s3");
    axios.get(`${Global.backend}/room/`)
      .then(response => {
        var flag = response.status === 200;
        console.log(response);
        this.valid = flag;
        if (flag) {
          this.allrooms = response.data;
        }
      })
      .catch(error => {
        console.log(error);
        this.valid = false;
      });
  },
};
</script>

<template>
  <header>
    <transition name="nav_bar" appear>
      <nav class="navbar navbar-light navbar-expand-md bg-faded justify-content-center bg-light fixed-top"
        v-show="isShow">
        <div class="container">
          <img class="logo" src="./assets/GHepherd_Logo_small.png" />
          <span class="navbar-brand d-flex w-25 me-auto"> GShepherd </span>
          <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#collapsingNavbar3">
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="navbar-collapse collapse w-100" id="collapsingNavbar3">
            <ul class="navbar-nav w-100 justify-content-center">
              <li class="nav-item active">
                <a class="nav-link" href="#">首頁</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#s2">介紹</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#s3">聯絡我們</a>
              </li>
            </ul>
            <ul class="nav navbar-nav ms-auto w-100 justify-content-end d-flex">
              <div class="input-group">
                <input type="text" class="form-control" placeholder="room ID here:" v-model="roomID2" />
                <button class="btn btn-outline-secondary" type="button" @click="find_room_and_redirect(roomID2)">
                  GO!
                </button>
                <a :href="`/login/`">
                  <button class="btn btn-success" type="button">
                    <span class="material-symbols-outlined d-flex">
                      person
                    </span>
                  </button>
                </a>
              </div>
            </ul>
          </div>
        </div>
      </nav>
    </transition>
  </header>
  <main>
    <section class="sec">
      <div class="d-flex">
        <transition name="slide_1" appear>
          <div class="w-75" id="s1" v-show="isShow">
            <span class="welcom">Welcome GShepherd!</span>
          </div>
        </transition>
        <div class="w-25 bar d-flex justify-content-center align-items-center" id="b1">
          <transition name="slide_1_bar" appear>
            <div v-show="isShow">
              <h2>哈囉，第一次來ㄇ?</h2>
              請輸入房間#ID<br />
              <input class="form-control me-2" type="search" aria-label="Search" v-model="roomID" />
              <button class="btn btn-outline-secondary" @click="find_room_and_redirect(roomID)">
                <span class="material-symbols-outlined d-flex">
                  subdirectory_arrow_right
                </span>
              </button>
              <br />
              或是...開啟新房間?<br />
              <a href="/login/" style="text-decoration: none">
                <button class="btn btn-success d-flex">
                  註冊/登入
                </button>
              </a>
            </div>
          </transition>
        </div>
      </div>
    </section>
    <section class="sec">
      <div class="d-flex">
        <div class="w-75" id="s2">
          <div><span class="welcom">What We Have?</span></div>
          <transition name="slide_2">
            <div v-show="isShow_s2">
              <div style="display: flex; justify-content: left; margin-left: 6vw">
                <div class="feature">
                  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.88 122.34"
                    style="enable-background: new 0 0 122.88 122.34" xml:space="preserve">
                    <g>
                      <path fill="#ffffff"
                        d="M58.61,28.13c-1.3,0-2.35-1.05-2.35-2.35c0-1.3,1.05-2.35,2.35-2.35h16.22c1.3,0,2.35,1.05,2.35,2.35 c0,1.3-1.05,2.35-2.35,2.35H58.61L58.61,28.13z M85.5,25.16h22.19c0.97,0,1.8,0.59,2.16,1.42l12.57,24.27 c0.2,0.38,0.28,0.8,0.26,1.2l0.21,67.94c0,1.29-1.05,2.34-2.34,2.34v0.01H2.35c-1.3,0-2.35-1.05-2.35-2.35V51.95l0-0.02l0-0.02 l0-0.02v0l0-0.02l0-0.02l0-0.02l0-0.01l0-0.02l0-0.03v0l0-0.01l0-0.04l0-0.01l0-0.01l0-0.04l0-0.01l0-0.01l0.01-0.04l0,0l0-0.02 l0.01-0.04l0,0l0,0l0.01-0.04l0-0.02v0l0.01-0.04l0.01-0.02l0-0.02l0-0.02l0-0.01l0-0.01l0.01-0.02l0.01-0.02l0-0.01l0-0.01 l0.01-0.02l0.01-0.02l0-0.01l0-0.01l0.01-0.02l0-0.01l0.01-0.03l0.01-0.02l0.01-0.02l0.01-0.02l0.01-0.02l0.01-0.02v0l0.01-0.02 l0.01-0.02v0l0.02-0.04L0.2,51l0.02-0.04l0,0l0.01-0.02l0.01-0.01l0-0.01l0.01-0.02l0.01-0.02l0.01-0.02l0.01-0.02l0.02-0.04 L0.3,50.8l0,0l0.02-0.03l0.01-0.02l0.01-0.02l0.01-0.02l12.78-24.3c0.42-0.8,1.23-1.25,2.07-1.25v-0.01h21.7V2.91 c0-0.8,0.33-1.53,0.85-2.06C38.28,0.33,39.01,0,39.81,0h42.78c0.8,0,1.53,0.33,2.06,0.85c0.53,0.53,0.85,1.25,0.85,2.06V25.16 L85.5,25.16z M36.9,29.85H16.61L6.23,49.6H116.5l-10.23-19.75H85.5v7.35h5.17c1.3,0,2.35,1.05,2.35,2.35c0,1.3-1.05,2.35-2.35,2.35 h-7.52H39.25h-6.3c-1.3,0-2.35-1.05-2.35-2.35c0-1.3,1.05-2.35,2.35-2.35h3.96V29.85L36.9,29.85z M4.69,54.3v63.35h113.5L118,54.3 H4.69L4.69,54.3z M80.81,4.69H41.59V37.2h39.22V4.69L80.81,4.69z M47.58,17.11c-1.3,0-2.35-1.05-2.35-2.35 c0-1.3,1.05-2.35,2.35-2.35h27.24c1.3,0,2.35,1.05,2.35,2.35c0,1.3-1.05,2.35-2.35,2.35H47.58L47.58,17.11z" />
                    </g>
                  </svg>
                  <p>投投票</p>
                </div>
                <div class="feature">
                  <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 112.42 122.88">
                    <title>light</title>
                    <path
                      d="M33.37,82.31C31.92,80.65,30.44,79,29,77a38.93,38.93,0,0,1-4.09-6.82,40,40,0,0,1-2.78-7.9,35,35,0,0,1-.93-8.25h0a34.77,34.77,0,0,1,1.14-8.54,41.42,41.42,0,0,1,3.38-8.72l.13-.22a33.92,33.92,0,0,1,7-8.43A32.44,32.44,0,0,1,42,22.48l.19-.07a34.63,34.63,0,0,1,7.92-2.12,37.29,37.29,0,0,1,8.58-.2,38.37,38.37,0,0,1,8.25,1.69,37.89,37.89,0,0,1,7.63,3.41A34.86,34.86,0,0,1,83,32.1a31.59,31.59,0,0,1,5.59,9h0a37.62,37.62,0,0,1,1.82,5.81,34.14,34.14,0,0,1,.23,13.69,38,38,0,0,1-2.12,7.16C85.8,74.43,82.3,78.53,79,82.35L72.17,93.26l-.83,3.88-.24,3.2.08,4.14a25.15,25.15,0,0,1,0,4.16A15,15,0,0,1,70.1,113l0,0c-.16.41-.31.77-.46,1.08A15.78,15.78,0,0,1,62,121.6a13.8,13.8,0,0,1-10.75.36q-.55-.21-1-.45l-.16-.08a12.51,12.51,0,0,1-4.62-3.79l-.12-.17a24.63,24.63,0,0,1-2.49-4.67,3.22,3.22,0,0,1-.16-.5,35.35,35.35,0,0,1-.81-4.22c-.14-1.22-.18-2.48-.21-4.34v0a34.08,34.08,0,0,0-2.48-11.28h0a34.62,34.62,0,0,0-5.74-10.11Zm14.34,29.88,18.56-2.76a12.13,12.13,0,0,0,.18-1.23,21.56,21.56,0,0,0,0-3.41l0-1.11-20,3c0,.3.05.6.08.89a27.57,27.57,0,0,0,.68,3.52c.17.4.34.77.51,1.13Zm16.33,2-13.54,2a8.67,8.67,0,0,0,1.62,1l.11,0c.25.12.48.23.7.31a9.15,9.15,0,0,0,7.1-.28,11,11,0,0,0,4-3.13ZM46.28,102.3l20.14-3,.21-2.67,0-.31.39-1.82L45.61,97.69a33.29,33.29,0,0,1,.67,4.61ZM44.5,93.47l3.59-.53L37,57.71a2.07,2.07,0,0,1,4-1.24l.86,2.74a2.35,2.35,0,0,1,.24-.6L50.42,44.4a2.06,2.06,0,0,1,2.83-.74,2.1,2.1,0,0,1,.85,1l2.73,6.25,2.77-7a2.07,2.07,0,0,1,2.69-1.16A2,2,0,0,1,63.51,44l5,13.25a2.07,2.07,0,0,1-3.88,1.47l-3.17-8.35L58.86,57a2.1,2.1,0,0,1-1.11,1.14A2.07,2.07,0,0,1,55,57.09L51.93,50,45.64,60.71a2.07,2.07,0,0,1-2.83.73,1.83,1.83,0,0,1-.36-.26L52.14,92a2.05,2.05,0,0,1,.07.34l7.45-1.1L69.47,57.1a2.07,2.07,0,1,1,4,1.14l-9.28,32.3,4.55-.67L75,79.79a3.11,3.11,0,0,1,.25-.38l0-.06C78.4,75.79,81.68,72,84.1,66A33.21,33.21,0,0,0,86,59.71a28.69,28.69,0,0,0,.5-6.23,29.36,29.36,0,0,0-.69-5.57,31.91,31.91,0,0,0-1.6-5h0a26.69,26.69,0,0,0-4.76-7.61,29.83,29.83,0,0,0-7.26-6,32.46,32.46,0,0,0-6.67-3,33.2,33.2,0,0,0-7.22-1.48,31.82,31.82,0,0,0-7.49.18A29.43,29.43,0,0,0,44,26.79l-.18.09a27.71,27.71,0,0,0-7.84,4.82,29.55,29.55,0,0,0-6,7.28,37,37,0,0,0-3,7.71,30.43,30.43,0,0,0-1,7.39,29.78,29.78,0,0,0,.82,7.15,35.86,35.86,0,0,0,2.44,7,34.39,34.39,0,0,0,3.61,6c1.33,1.8,2.73,3.41,4.11,5a39.63,39.63,0,0,1,6.62,11.55h0c.35.9.66,1.82,1,2.73ZM3.59,60A3.53,3.53,0,0,1,0,56.55a3.52,3.52,0,0,1,3.42-3.63l8.43-.29A3.52,3.52,0,0,1,15.48,56a3.51,3.51,0,0,1-1,2.53,3.6,3.6,0,0,1-2.47,1.11L3.59,60Zm105-11.29a3.51,3.51,0,0,1,3.83,3.15V52a3.51,3.51,0,0,1-3.16,3.72l-8.39.88a3.53,3.53,0,0,1-.72-7l8.43-.88ZM92.18,15l.09-.13a3.56,3.56,0,0,1,2.16-1.37,3.51,3.51,0,0,1,2.63.53l.14.09a3.62,3.62,0,0,1,1.37,2.16A3.56,3.56,0,0,1,98,18.9l-4.69,7a3.53,3.53,0,0,1-2.24,1.5H91A3.52,3.52,0,0,1,87,24.63,3.49,3.49,0,0,1,87.49,22l4.69-7ZM55.82,3.48a.79.79,0,0,1,0-.22A3.51,3.51,0,0,1,59.39,0h.22A3.57,3.57,0,0,1,61.9,1.1a3.53,3.53,0,0,1,1,2.47.91.91,0,0,1,0,.17L62.65,12a.74.74,0,0,1,0,.21,3.49,3.49,0,0,1-3.56,3.26.74.74,0,0,1-.21,0,3.49,3.49,0,0,1-3.26-3.56.48.48,0,0,1,0-.17l.21-8.25ZM13.92,17.61a3.53,3.53,0,0,1-.19-5,3.54,3.54,0,0,1,5-.18l6.17,5.75a3.54,3.54,0,0,1,.18,5,3.53,3.53,0,0,1-5,.18l-6.17-5.75Z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>想想看</p>
                </div>
                <div class="feature">
                  <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                    clip-rule="evenodd" viewBox="0 0 512 486.31">
                    <path
                      d="M98.69 41.81h53.45C155.01 18.24 174.04 0 197.13 0c22.94 0 41.87 17.98 44.93 41.32l54.12.49c2.76 0 4.98 2.22 4.98 5v55.01a4.97 4.97 0 0 1-4.98 4.99H98.75c-2.72 0-5-2.22-5-4.99V46.81a4.92 4.92 0 0 1 4.94-5zm347.57 25.98h53.01c6.97 0 12.71 5.72 12.71 12.71l.02 29.32-78.46.01.01-29.33c0-6.97 5.73-12.71 12.71-12.71zm65.63 344.48c-7.32 13.88-14.65 27.79-21.98 41.69-17.22 32.67-11.98 37.69-29.19 6.88l-27.09-48.57-.08.01v-287.6l78.43-.02v287.62l-.09-.01zm-64.88-.02 51.5.02c-4.82 9.11-9.64 18.24-14.45 27.35-11.35 21.45-7.89 24.76-19.2 4.53l-17.85-31.9zM145.14 206.54c-7.57 0-13.71-6.14-13.71-13.72 0-7.57 6.14-13.71 13.71-13.71H296.7c7.57 0 13.72 6.14 13.72 13.71 0 7.58-6.15 13.72-13.72 13.72H145.14zm0 78.48c-7.57 0-13.71-6.15-13.71-13.72s6.14-13.72 13.71-13.72H296.7c7.57 0 13.72 6.15 13.72 13.72s-6.15 13.72-13.72 13.72H145.14zm-45.67 49.46c8.47 0 15.32 6.86 15.32 15.32 0 8.47-6.85 15.33-15.32 15.33s-15.32-6.86-15.32-15.33c0-8.46 6.85-15.32 15.32-15.32zm45.67 29.04c-7.57 0-13.71-6.15-13.71-13.72s6.14-13.72 13.71-13.72h133.79c7.57 0 13.72 6.15 13.72 13.72s-6.15 13.72-13.72 13.72H145.14zM99.47 255.98c8.47 0 15.32 6.86 15.32 15.33 0 8.46-6.85 15.32-15.32 15.32s-15.32-6.86-15.32-15.32c0-8.47 6.85-15.33 15.32-15.33zm0-78.48c8.47 0 15.32 6.85 15.32 15.32s-6.85 15.32-15.32 15.32-15.32-6.85-15.32-15.32S91 177.5 99.47 177.5zM44.27 62.91h25.4v25.86h-25.4c-5.06 0-9.69 2.07-13.02 5.39-3.31 3.33-5.39 7.95-5.39 13.01v334.85c0 5.05 2.09 9.68 5.4 13.02 3.34 3.32 7.97 5.4 13.01 5.4h306.35c4.79 0 9.18-1.86 12.44-4.88l.54-.56c3.35-3.35 5.44-7.96 5.44-12.98V107.17c0-5.05-2.06-9.66-5.38-12.98-3.33-3.33-7.97-5.42-13.04-5.42h-25.39V62.91h25.39c12.16 0 23.21 5 31.22 12.99 8.06 8.05 13.07 19.14 13.07 31.27v334.85c0 12.17-5 23.25-13.02 31.27l-.89.81c-7.96 7.55-18.69 12.21-30.38 12.21H44.27c-12.12 0-23.19-5-31.22-13.01C5 465.23 0 454.17 0 442.02V107.17c0-12.14 4.99-23.2 12.99-31.22 8.07-8.05 19.14-13.04 31.28-13.04zm152.34-41.52c13.04 0 23.62 10.58 23.62 23.62 0 13.06-10.58 23.64-23.62 23.64-13.05 0-23.63-10.58-23.63-23.64 0-13.04 10.58-23.62 23.63-23.62z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>測測驗</p>
                </div>
              </div>
              <div style="display: flex; justify-content: left; margin-left: 6vw">
                <div class="feature">
                  <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                    clip-rule="evenodd" viewBox="0 0 512 485.95">
                    <path fill-rule="nonzero"
                      d="m342.29 92.77 22.44 52.57 57.18 5.12c2.3.23 4.3 1.37 5.68 3.01l.23.31a8.46 8.46 0 0 1 1.76 5.78l-.03.32a8.57 8.57 0 0 1-.88 2.95l-.13.24c-.41.75-.94 1.43-1.55 2.02l-43.49 37.99 12.81 56.09c.46 2.23-.01 4.45-1.14 6.25a8.51 8.51 0 0 1-8.71 3.86 8.242 8.242 0 0 1-3.2-1.25l-48.9-29.24-49.3 29.47c-2 1.16-4.29 1.42-6.35.9a8.554 8.554 0 0 1-5.25-3.89l-.17-.32a8.522 8.522 0 0 1-.83-5.99l12.76-55.88-43.17-37.69a8.556 8.556 0 0 1-2.9-5.88c-.14-2.16.54-4.39 2.1-6.17.78-.89 1.69-1.58 2.68-2.06 1.02-.5 2.14-.78 3.28-.85l56.77-5.09 22.52-52.71a8.566 8.566 0 0 1 4.68-4.58c2.01-.81 4.36-.86 6.53.07l.24.11c.96.44 1.79 1.03 2.47 1.71l.27.29c.66.74 1.19 1.57 1.56 2.46l.04.08zM0 259.96h106.66l-4.27 184.64H0V259.96zm123.5 169.73 3.57-153.26 68.93 1.61c29.1 5.92 57.95 22.44 86.75 41.53l53.54 1.24c24.2 2.03 36.32 26.88 12.4 42.47-19.1 13.33-43.85 11.98-69.18 9.11-17.49-1.27-18.8 22.25-.53 22.75 6.32.64 13.26-.68 19.28-.55 31.69.71 57.94-4.74 74.49-29.4l8.47-18.56L461.78 309c40.16-12.19 67.51 30.16 37.48 58.46-58.66 40.63-118.64 73.79-179.79 100.27-44.57 25.71-88.49 23.78-131.84-3.07l-64.13-34.97zm56.4-320.68c-4.09-1.72-6.01-6.44-4.29-10.53 1.72-4.09 6.44-6.01 10.53-4.29l33.57 14.22c4.09 1.72 6.01 6.44 4.29 10.53a8.056 8.056 0 0 1-10.54 4.29l-33.56-14.22zm34.13-52.31c-3.13-3.16-3.12-8.26.03-11.39 3.16-3.13 8.26-3.12 11.39.03l25.64 25.92c3.13 3.15 3.12 8.25-.03 11.38-3.16 3.14-8.26 3.12-11.39-.03L214.03 56.7zm51.46-35.44c-1.69-4.11.28-8.81 4.39-10.5 4.11-1.69 8.81.27 10.5 4.38l13.86 33.72a8.044 8.044 0 0 1-4.38 10.5c-4.11 1.69-8.82-.27-10.51-4.38l-13.86-33.72zm217.09 72.93c4.09-1.72 8.81.2 10.53 4.29 1.72 4.09-.19 8.81-4.29 10.53l-33.56 14.22c-4.09 1.72-8.81-.2-10.53-4.29a8.028 8.028 0 0 1 4.29-10.53l33.56-14.22zm-39.31-48.85c3.14-3.15 8.24-3.16 11.39-.03 3.15 3.13 3.17 8.23.03 11.39l-25.64 25.91c-3.13 3.15-8.23 3.17-11.38.03-3.15-3.13-3.17-8.23-.04-11.38l25.64-25.92zm-54.93-30.2c1.69-4.11 6.4-6.07 10.5-4.38 4.11 1.69 6.08 6.39 4.39 10.5l-13.86 33.72a8.044 8.044 0 0 1-10.5 4.38c-4.11-1.69-6.08-6.39-4.39-10.5l13.86-33.72zm-62.3-7.07c0-4.45 3.61-8.07 8.07-8.07 4.46 0 8.07 3.62 8.07 8.07v36.46c0 4.45-3.61 8.07-8.07 8.07-4.46 0-8.07-3.62-8.07-8.07V8.07z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>評評分</p>
                </div>
                <div class="feature">
                  <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                    clip-rule="evenodd" viewBox="0 0 512 450.27">
                    <path
                      d="M217.91 393.59c53.26 49.01 127.33 63.27 201.39 31.71l63.49 24.97-9.94-59.73c59.07-51.65 45.36-123.42-1.79-173.93-3.69 19.53-10.48 38.07-19.94 55.27-14.17 25.77-34.46 48.67-59.31 67.52-24.07 18.27-52.17 32.61-82.8 41.87-28.16 8.51-58.91 12.91-91.1 12.32zm-85.88-167.22c-7.7 0-13.95-6.25-13.95-13.95 0-7.7 6.25-13.95 13.95-13.95h124.12c7.7 0 13.94 6.25 13.94 13.95 0 7.7-6.24 13.95-13.94 13.95H132.03zm0-71.41c-7.7 0-13.95-6.25-13.95-13.95 0-7.71 6.25-13.95 13.95-13.95h177.35c7.7 0 13.94 6.24 13.94 13.95 0 7.7-6.24 13.95-13.94 13.95H132.03zM226.13.12l.21.01c60.33 1.82 114.45 23.27 153.19 56.49 39.57 33.92 63.3 80.1 61.82 130.51l-.01.23c-1.56 50.44-28.05 95.17-69.62 126.71-40.74 30.92-96.12 49.16-156.44 47.39-15.45-.46-30.47-2.04-44.79-4.82-12.45-2.42-24.5-5.75-36-10.05L28.17 379.06l31.85-75.75c-18.2-15.99-32.94-34.6-43.24-55.01C5.29 225.51-.72 200.48.07 174.33c1.52-50.49 28.02-95.26 69.61-126.82C110.44 16.59 165.81-1.65 226.13.12zm-.55 27.7-.21-.01C171.49 26.23 122.33 42.3 86.41 69.55c-35.07 26.61-57.39 63.9-58.65 105.54-.65 21.39 4.31 41.94 13.78 60.72 10.01 19.82 25.02 37.7 43.79 52.58l8.26 6.54-16.99 40.39 59.12-18.06 4.5 1.81c11.15 4.48 23.04 7.9 35.48 10.31 13.07 2.55 26.59 3.98 40.34 4.39 53.88 1.58 103.04-14.49 138.96-41.74 35.07-26.61 57.39-63.9 58.65-105.54v-.22c1.19-41.57-18.82-80.01-52.15-108.59-34.18-29.3-82.19-48.24-135.92-49.86z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>討討論</p>
                </div>
                <div class="feature">
                  <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                    clip-rule="evenodd" viewBox="0 0 512 421.65">
                    <path fill-rule="nonzero"
                      d="M383.01 0c11.57 0 20.97 9.4 20.97 20.97 0 11.57-9.4 20.97-20.97 20.97l-362.04.44C9.4 42.38 0 32.97 0 21.4 0 9.84 9.4.43 20.97.43L383.01 0zM263.22 309.53c-7.91-7.95-7.86-20.83.08-28.74s20.83-7.86 28.74.08l71.48 71.5.32-206.63c0-11.2 9.09-20.3 20.29-20.3s20.3 9.1 20.3 20.3l-.32 206.4 73.12-73.12c7.94-7.94 20.87-7.94 28.81 0 7.95 7.95 7.95 20.88 0 28.82l-107.9 107.9c-8.02 7.91-20.9 7.87-28.81-.08L263.22 309.53zm-91.6 29.58c11.57 0 20.97 9.4 20.97 20.97 0 11.57-9.4 20.97-20.97 20.97l-150.65.16C9.4 381.21 0 371.81 0 360.24c0-11.56 9.4-20.97 20.97-20.97l150.65-.16zm41.33-170.7c11.57 0 20.97 9.4 20.97 20.97 0 11.57-9.4 20.97-20.97 20.97l-191.98.23C9.4 210.58 0 201.17 0 189.6c0-11.56 9.4-20.97 20.97-20.97l191.98-.22z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>比比看</p>
                </div>
              </div>
              <div style="display: flex; justify-content: left; margin-left: 6vw">
                <div class="feature">
                  <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103.68 122.88">
                    <title>new-product</title>
                    <path
                      d="M50.67,81.17,15.21,62.53,5.81,73.89c12.58,6.65,24.91,13.31,37.5,20l7.36-12.68ZM23.06,15.3h.25A7.22,7.22,0,0,1,25.56,10a7.22,7.22,0,0,1,5.27-2.25V7.53a7.19,7.19,0,0,1-5.27-2.26A7.26,7.26,0,0,1,23.31,0h-.25a7.22,7.22,0,0,1-2.25,5.27,7.22,7.22,0,0,1-5.28,2.26v.24A7.26,7.26,0,0,1,20.81,10a7.22,7.22,0,0,1,2.25,5.27Zm10.53,4.36h.11a3.62,3.62,0,0,1,3.53-3.53V16A3.4,3.4,0,0,1,34.76,15a3.42,3.42,0,0,1-1.06-2.47h-.11A3.38,3.38,0,0,1,32.53,15,3.36,3.36,0,0,1,30.06,16v.12a3.6,3.6,0,0,1,3.53,3.52Zm-20,7.72h.2A6.07,6.07,0,0,1,15.7,23a6.09,6.09,0,0,1,4.43-1.89v-.21A6.09,6.09,0,0,1,15.7,19a6.09,6.09,0,0,1-1.89-4.43h-.2A6.08,6.08,0,0,1,11.71,19a6,6,0,0,1-4.42,1.89v.21A6.07,6.07,0,0,1,11.71,23a6.08,6.08,0,0,1,1.9,4.43Zm68.6-12H82a7.23,7.23,0,0,0-2.26-5.27,7.22,7.22,0,0,0-5.27-2.25V7.57a7.19,7.19,0,0,0,5.27-2.26A7.27,7.27,0,0,0,82,0h.24a7.19,7.19,0,0,0,2.26,5.27,7.19,7.19,0,0,0,5.27,2.26v.24a7.23,7.23,0,0,0-5.27,2.26,7.19,7.19,0,0,0-2.26,5.27ZM71.68,19.7h-.11A3.62,3.62,0,0,0,68,16.17v-.11A3.4,3.4,0,0,0,70.51,15a3.36,3.36,0,0,0,1.06-2.46h.11A3.38,3.38,0,0,0,72.74,15a3.38,3.38,0,0,0,2.47,1.06v.12a3.6,3.6,0,0,0-3.53,3.52Zm20,7.09h-.19a5.84,5.84,0,0,0-5.7-5.7V20.9a5.45,5.45,0,0,0,4-1.71,5.47,5.47,0,0,0,1.71-4h.19a5.5,5.5,0,0,0,1.7,4,5.5,5.5,0,0,0,4,1.7v.19a5.83,5.83,0,0,0-5.69,5.7ZM53.1,11l6.72,15.69,17.05,1.53a1.36,1.36,0,0,1,1.26,1.36,1.35,1.35,0,0,1-.48,1h0L64.74,41.86l3.81,16.68a1.53,1.53,0,0,1,0,.3,1.35,1.35,0,0,1-1.06,1.33l-.29,0a1.41,1.41,0,0,1-.77-.22L51.81,51.24,37.13,60a1.36,1.36,0,0,1-1.88-.47,1.32,1.32,0,0,1-.19-.7,1.63,1.63,0,0,1,0-.31h0l3.82-16.68L26,30.61a1.33,1.33,0,0,1,.85-2.36l17-1.52L50.56,11a1.39,1.39,0,0,1,2.54,0ZM28.83,55.44l-11,5.46,34.7,18.23L87.28,60.86,74.53,54.19l-.79-3.46,14.81,7.61L99.48,48,80.14,37.48l2-1.73,0,0,.31-.29L103,46.31a1.27,1.27,0,0,1,.33.24,1.34,1.34,0,0,1,0,1.88l-12,11.74L102.9,74a1.32,1.32,0,0,1-.17,1.87,1.27,1.27,0,0,1-.27.18L92.08,81.56v20.59a1.35,1.35,0,0,1-.83,1.23L53.7,122.44a1.35,1.35,0,0,1-1,.44,1.33,1.33,0,0,1-1.19-.74l-37.63-18.8a1.35,1.35,0,0,1-.73-1.19V81.66L2.57,76.07a2.11,2.11,0,0,1-.27-.18A1.33,1.33,0,0,1,2.13,74L13.4,60.53.33,48.14a1.33,1.33,0,0,1,.12-1.88,2,2,0,0,1,.26-.18L20.63,34.81a8,8,0,0,0,.85.89h0L23,37,4,47.8l11.9,11L29.61,52l-.78,3.44Zm61,7.09L54.37,81.17,61.8,94,99.07,73.73l-9.24-11.2Z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>抽抽獎</p>
                </div>
                <div class="feature">
                  <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 81.28 122.88">
                    <title>payment</title>
                    <path class="cls-1"
                      d="M0,49.17,49.6,0,81.28,31.13l-5.11,2.38-22-21.59-.26-.27a5.06,5.06,0,0,0-7.15.16l-1.79,1.86-.06-.06-8.4,8.9L4.34,53.42,0,49.17ZM13.41,76.85l-.3,1.92c-.79,5.12-2.07,13.44-2.59,16.38a2.18,2.18,0,0,1-.17.51,1.29,1.29,0,0,1-.26.42,19.68,19.68,0,0,0-1.61,2.39,6.65,6.65,0,0,0-.76,1.89,2.23,2.23,0,0,0,0,1,1.73,1.73,0,0,0,.48.79C13.08,107.05,18.26,112,23,117a6.54,6.54,0,0,0,5.34,1.68,31.75,31.75,0,0,0,5.3-1.15,43.8,43.8,0,0,0,5.54-1.61,15.85,15.85,0,0,0,4.9-3l4.08-4.26a1.37,1.37,0,0,1,.19-.22l0,0c.12-.12.47-.47,1-.94l0,0c2.7-2.64,6-5.89,4.13-8.59l-1.37-1.37-1,.9L49.8,99.53c-.7.61-1.35,1.19-1.94,1.77a2.11,2.11,0,0,1-3-3c.63-.63,1.39-1.31,2.18-2l.1-.08c2.65-2.35,5.68-5,4.2-7.2l-1.61-1.61c-.38.39-.78.76-1.18,1.13s-1,.92-1.52,1.36L46.9,90c-.68.6-1.32,1.16-1.91,1.75a2.11,2.11,0,0,1-3-3c.61-.61,1.36-1.28,2.15-2l.12-.11c2.65-2.34,5.68-5,4.21-7.2-.56-.55-1.13-1.1-1.67-1.67l-4.48,4.48a2.11,2.11,0,0,1-3-3l9-9a7.51,7.51,0,0,0,1.89-2.88,4.36,4.36,0,0,0,.08-2.8,3.8,3.8,0,0,0-.37-.79,3.69,3.69,0,0,0-.52-.66,3.48,3.48,0,0,0-.67-.53,4,4,0,0,0-.78-.36,4.34,4.34,0,0,0-2.79.1,8.14,8.14,0,0,0-2.94,2L20.59,86.06a2.11,2.11,0,0,1-3-3l.94-.93-5.16-5.27Zm10.15.28L36.42,64.27l-.24-.07A9.18,9.18,0,1,1,47.87,58a8.27,8.27,0,0,1,1.41.33h0a8,8,0,0,1,1.65.76,8.1,8.1,0,0,1,1.45,1.14l0,0a7.7,7.7,0,0,1,.54.6L65.12,48.41a6,6,0,0,1,.1-8.43L54.06,28.58a6,6,0,0,1-8.44-.09h0L15,59.9A6,6,0,0,1,15,68.33l8.61,8.8Zm30.78-8.58v0a11.7,11.7,0,0,1-3,4.85l-1.47,1.46,1.74,1.74.16.18.14.19a6.16,6.16,0,0,1,.52,7.14l.05,0a2.91,2.91,0,0,1,.33.26l1.7,1.71a.93.93,0,0,1,.16.18l.13.17a6.48,6.48,0,0,1,1.42,4.23A7,7,0,0,1,55,94.19l1.69,1.68.16.19.12.17c4.14,5.66-.68,10.37-4.57,14.17-.26.31-.69.69-1,1-1.36,1.42-3,3.34-4.4,4.6-3.92,3.55-7.78,4.5-12.25,5.59a32.67,32.67,0,0,1-6.12,1.27,10.61,10.61,0,0,1-8.52-2.9L5.33,105.18a5.8,5.8,0,0,1-1.59-2.55,6.49,6.49,0,0,1-.08-3.08v0a10.41,10.41,0,0,1,1-2.74A19.92,19.92,0,0,1,6.52,93.9c.38-2.17,1.21-7.7,2-12.78.45-3.05.89-5.92,1.22-8L1.43,64.63,50.6,15h0L78.83,43.85,54.34,68.55Z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>刮刮樂</p>
                </div>
                <div class="feature">
                  <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                    clip-rule="evenodd" viewBox="0 0 512 506.49">
                    <path fill-rule="nonzero"
                      d="M294.24 17.11C294.24 7.69 303.53 0 315.1 0c11.59 0 20.87 7.64 20.87 17.11v74.84c0 9.43-9.28 17.12-20.87 17.12-11.57 0-20.86-7.65-20.86-17.12V17.11zm47.99 330.77 22.56 21.37 44.71-45.4c4.18-4.25 6.78-7.63 11.93-2.35l16.71 17.08c5.46 5.42 5.18 8.58.01 13.66l-63.85 62.74c-10.88 10.68-9 11.35-20.05.36l-40.14-39.9c-2.28-2.5-2.04-5 .45-7.52l19.38-20.07c2.93-3.04 5.26-2.85 8.29.03zm35.59-109.77c36.98 0 70.56 15.04 94.83 39.34 24.31 24.25 39.35 57.8 39.35 94.86 0 36.99-15.04 70.56-39.3 94.83-24.32 24.31-57.89 39.35-94.88 39.35-37.03 0-70.56-15.04-94.84-39.3-24.32-24.27-39.34-57.86-39.34-94.88 0-37.06 15.04-70.61 39.31-94.89l.69-.63c24.24-23.9 57.53-38.68 94.18-38.68zm78.74 55.41c-20.09-20.11-47.96-32.59-78.74-32.59-30.5 0-58.14 12.26-78.19 32.03l-.55.59c-20.15 20.15-32.62 48.01-32.62 78.76 0 30.75 12.46 58.6 32.61 78.75 20.1 20.13 47.98 32.6 78.75 32.6 30.76 0 58.65-12.47 78.77-32.59 20.11-20.11 32.58-48 32.58-78.76 0-30.75-12.47-58.61-32.61-78.79zM56.81 242.28c-1.18 0-2.24-5.2-2.24-11.57 0-6.38.94-11.53 2.24-11.53h56.94c1.19 0 2.25 5.2 2.25 11.53 0 6.39-.94 11.57-2.25 11.57H56.81zm90.78 0c-1.19 0-2.24-5.2-2.24-11.57 0-6.38.93-11.53 2.24-11.53h56.94c1.18 0 2.24 5.2 2.24 11.53 0 6.39-.94 11.57-2.24 11.57h-56.94zm90.77 0c-1.18 0-2.24-5.2-2.24-11.57 0-6.38.93-11.53 2.24-11.53h56.94c1.18 0 2.24 5.15 2.24 11.49a175.09 175.09 0 0 0-16.44 11.61h-42.74zM56.94 308.52c-1.18 0-2.24-5.2-2.24-11.57 0-6.39.93-11.59 2.24-11.59h56.94c1.18 0 2.24 5.2 2.24 11.59 0 6.37-.93 11.57-2.24 11.57H56.94zm90.77 0c-1.18 0-2.24-5.2-2.24-11.57 0-6.39.93-11.59 2.24-11.59h56.94c1.18 0 2.24 5.2 2.24 11.59 0 6.37-.93 11.57-2.24 11.57h-56.94zM57.06 374.8c-1.18 0-2.24-5.2-2.24-11.59 0-6.37.94-11.57 2.24-11.57H114c1.19 0 2.25 5.2 2.25 11.57 0 6.39-.94 11.59-2.25 11.59H57.06zm90.78 0c-1.19 0-2.25-5.2-2.25-11.59 0-6.37.94-11.57 2.25-11.57h56.94c1.18 0 2.24 5.2 2.24 11.57 0 6.39-.94 11.59-2.24 11.59h-56.94zM106.83 17.11c0-9.42 9.29-17.11 20.86-17.11 11.58 0 20.86 7.64 20.86 17.11v74.84c0 9.43-9.32 17.12-20.86 17.12-11.57 0-20.86-7.65-20.86-17.12V17.11zM22.98 163.64h397.39V77.46c0-2.94-1.18-5.53-3.08-7.43-1.9-1.9-4.61-3.08-7.44-3.08h-38.1c-6.37 0-11.57-5.2-11.57-11.57 0-6.38 5.2-11.58 11.57-11.58h38.1c9.34 0 17.7 3.77 23.82 9.88 6.13 6.14 9.88 14.5 9.88 23.83v136.81c-7.59-2.62-15.41-4.73-23.43-6.29v-21.38h.25H22.98v223.16c0 2.95 1.19 5.53 3.08 7.43 1.9 1.9 4.61 3.08 7.43 3.08h188.86c2.15 8.02 4.86 15.84 8.12 23.36H33.71c-9.3 0-17.7-3.75-23.82-9.89C3.77 427.72 0 419.36 0 410.02V77.55c0-9.29 3.77-17.7 9.89-23.82 6.12-6.13 14.48-9.89 23.82-9.89h40.68c6.37 0 11.57 5.2 11.57 11.57 0 6.39-5.2 11.59-11.57 11.59H33.71c-2.96 0-5.53 1.18-7.43 3.08-1.91 1.9-3.1 4.59-3.1 7.43v86.16h-.2v-.03zm158.95-96.69c-6.38 0-11.58-5.2-11.58-11.57 0-6.38 5.2-11.58 11.58-11.58h77.54c6.39 0 11.59 5.2 11.59 11.58 0 6.37-5.2 11.57-11.59 11.57h-77.54z"
                      fill="#ffffff"></path>
                  </svg>
                  <p>選選日</p>
                </div>
              </div>
            </div>
          </transition>
        </div>
        <div class="w-25 bar d-flex justify-content-center align-items-center" id="b2" ref="b2">
          <transition name="slide_2_bar">
            <div v-show="isShow_s2">
              <h2>即時線上互動，教學好有趣!</h2>
              <ul class="list-group">
                <li class="list-group-item active">在這裡，你可以...</li>
                <li class="list-group-item">立即開啟房間，加入群眾開始活動</li>
                <li class="list-group-item">預定活動，與觀眾互動不卡卡</li>
                <li class="list-group-item">活動結果即時呈現，讓氣氛嗨起來</li>
              </ul>
            </div>
          </transition>
        </div>
      </div>
    </section>
    <section class="sec">
      <div class="d-flex">
        <div class="w-75" id="s3">
          <span class="welcom">Contact Us!</span>
          <transition name="slide_3">
            <div style="display: flex; justify-content: center" v-show="isShow_s3">
              <div class="profile">
                <img src="/src/assets/profile/un_tyson_192b.png" />
                <h3 style="margin: 2vh">楊大為</h3>
                <p><a style="margin: 2vh">testzzo.no1@gmail.com</a></p>
                <a href="https://kotno557.github.io/Internet-fall-2022/" target="_blank">
                  <button class="btn btn-info" type="submit">個人頁面</button>
                </a>
              </div>
              <div class="profile">
                <img src="https://internet.nizw0.com/static/media/profile.27bb6496e46aef6d68c9.png" />
                <h3 style="margin: 2vh">吳丞恩</h3>
                <p><a style="margin: 2vh">ce.wu@outlook.com</a></p>
                <a href="https://internet.nizw0.com/" target="_blank">
                  <button class="btn btn-info" type="submit">個人頁面</button>
                </a>
              </div>
            </div>
          </transition>
        </div>

        <div class="w-25 bar d-flex justify-content-center align-items-center">
          <transition name="slide_3_bar">
            <div id="b3" v-show="isShow_s3">
              <div>
                <h2>遇到問題就聯絡我們ㄅ!</h2>
              </div>
              <div class="card">
                <p>人生難免都會遇到問題。</p>
                <p>若在這裡碰到問題就趕緊連絡我們ㄅ!</p>
                <p>工作人員將在兩到三個工作天之內回覆</p>
              </div>
            </div>
          </transition>
        </div>
      </div>
    </section>
  </main>

  <div v-if="true" class="modal fade" id="signin" tabindex="-1" role="dialog" aria-labelledby="signinLabel"
    aria-hidden="false">
    ...
    <div class="modal-content">
      <form @submit.prevent.stop="handleSubmit">
        ...
        <button type="submit"> Submit </button>
      </form>
    </div>
  </div>
</template>

<style scope>
@import url("../public/bootstrap-5.2.2/bootstrap-5.2.2-dist/css/bootstrap.css");

main {
  padding-top: 70px;
  background-color: #fafafa;
}

.logo {
  width: 50px;
}

.sec {
  height: 95vh;
}

#s1 {
  background-image: url(/src/assets/shepherd/pexels-rachel-6302312.jpg);
  background-size: cover;
  background-position: 40%;
  height: 95vh;
}

#s2 {
  background-image: url(/src/assets/shepherd/pexels-max-mishin-10026138.jpg);
  background-size: cover;
  background-position: right bottom;
  height: 95vh;
  scroll-margin-top: 5vh;
}

#s3 {
  background-image: url(/src/assets/shepherd/pexels-adam-kontor-333083.jpg);
  background-size: cover;
  background-position: left bottom;
  height: 95vh;
}

.welcom {
  padding: 2vh 2vw 2vh 2vw;
  margin: 4vh 2vw 2vh 2vw;
  font-size: 4vmin;
  font-weight: bold;
  color: white;
  background-color: rgba(112, 66, 20, 0.8);
  display: inline-block;
  font-family: "Raleway", sans-serif;
  border-radius: 5vh;
}

.bar {
  background-color: #fafafa;
  width: 30%;
  font-size: 2.5vmin;
}

header .w-25 {
  width: 45% !important;
}

#b1,
#b2 {
  box-sizing: border-box;
  border-bottom: solid rgb(224, 224, 224);
}

#b2 div {
  margin-top: 1vh;
  margin-left: 1vw;
  justify-content: center;
}

#b3 div {
  margin-top: 5vh;
  margin-left: 2vw;
  justify-content: center;
}

.feature {
  width: 10vw;
  height: 10vw;
  background-color: rgba(0, 0, 0, 0.7);
  margin: 2%;
  border-radius: 5vh;
  text-align: center;
  font-weight: bold;
  font-family: "Noto Sans TC", sans-serif;
  color: rgb(228, 227, 227);
  font-size: 2vmin;
}

.feature svg {
  margin-top: 26%;
  inline-size: 50%;
  height: 7vmin;
}

.profile {
  display: block;
  margin: 1em;
}

.profile img {
  margin-top: 15vh;
  width: 20vh;
  height: 20vh;
  border-radius: 50%;
}

.profile {
  width: 20vw;
  height: 70vh;
  background-color: rgba(0, 0, 0, 0.5);
  margin: 2%;
  border-radius: 5vh;
  text-align: center;
  font-family: "Noto Sans TC", sans-serif;
  color: rgb(228, 227, 227);
  font-size: 2.5vmin;
}

.boxhead .otherPage {
  text-decoration: none;
}

.slide_1-leave-active,
.slide_1-enter-active {
  transition: all 2s ease;
}

.slide_1-enter-from {
  transform: translateX(-30%);
}

.slide_1-leave-to {
  opacity: 0;
}

.slide_2-leave-active,
.slide_2-enter-active {
  transition: all 2s ease;
}

.slide_2-enter-from {
  transform: translateY(-30%);
}

.slide_2-leave-to {
  opacity: 0;
}

.slide_3-leave-active,
.slide_3-enter-active {
  transition: all 2s ease;
}

.slide_3-enter-from,
.slide_3-leave-to {
  opacity: 0;
}

.slide_1_bar-leave-active,
.slide_1_bar-enter-active {
  transition: all 1.5s ease;
}

.slide_1_bar-enter-from {
  transform: translateX(30%);
}

.slide_1_bar-leave-to {
  opacity: 0;
}

.slide_2_bar-leave-active,
.slide_2_bar-enter-active {
  transition: all 1.5s ease;
}

.slide_2_bar-enter-from {
  transform: translateY(50%);
}

.slide_2_bar-leave-to {
  opacity: 0;
}

.slide_3_bar-leave-active,
.slide_3_bar-enter-active {
  transition: opacity 1s;
}

.slide_3_bar-enter-from,
.slide_3_bar-leave-to {
  opacity: 0;
}

.nav_bar-leave-active,
.nav_bar-enter-active {
  transition: all 1s ease;
}

.nav_bar-enter-from {
  transform: translateY(-100%);
}

.nav_bar-leave-to {
  transform: translateY(100%);
}
</style>
